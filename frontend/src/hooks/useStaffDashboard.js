...
export const useStaffDashboard = () => {
  ...
  const snapshot = useMemo(() => {
    ...
    const floorsCovered = new Set(
      inventory.map((room) => room.floor).filter((value) => value !== null && value !== undefined)
    ).size;
    ...
    const guestCapacityReady = inventory.reduce(
      (max, room) => Math.max(max, Number(room.roomType?.maxGuests ?? 0)),
      0
    );

    const alerts = [];
    if (availableTonight === 0) {
      alerts.push({ type: 'noInventory' });
    } else if (availableTonight <= 3) {
      alerts.push({ type: 'tightInventory' });
    }

    if (premiumReady === 0 && availableTonight > 0) {
      alerts.push({ type: 'noPremium' });
    }
