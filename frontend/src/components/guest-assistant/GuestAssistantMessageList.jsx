    ...
    <div className="space-y-3">
      {messages.map((message, index) => {
        ...
        return (
          <div
            key={message.id}
            className={`motion-message-in flex min-w-0 ${isGuest ? 'motion-message-user justify-end' : 'motion-message-assistant justify-start'}`}
            style={{ '--motion-index': index }}
          >
            <div className={`min-w-0 max-w-[86%] rounded-[1.4rem] px-4 py-3 shadow-[0_18px_42px_-32px_rgba(15,23,42,0.58)] transition-transform duration-300 [transition-timing-function:var(--ease-spring-soft)] hover:scale-[1.01] ${bubbleClass}`}>
      ...
      {typingLabel ? (
        <div className="motion-message-in motion-message-assistant flex min-w-0 justify-start">
          <div className="motion-typing-bubble inline-flex min-w-0 items-center gap-2 rounded-full border border-brand-surface-border bg-white px-4 py-2 text-xs font-bold text-brand-ink-muted shadow-sm" aria-label={typingLabel}>
