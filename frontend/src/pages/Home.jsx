          ...
          <div className="grid min-w-0 items-start gap-8 lg:grid-cols-[1fr_1.15fr] lg:gap-12">
            ...
            <div className="flex min-w-0 flex-col justify-center lg:pt-4">
              ...
              <motion.div
                ...
                className="mt-6 flex min-w-0 flex-wrap gap-2.5"
              >
                  ...
                  <div className="min-w-0 flex-1 p-4 sm:p-5">
                    <div className="grid min-w-0 grid-cols-4 gap-2">
                      {[
                        ...
                        >
                          <p className="text-[9px] font-black uppercase tracking-wider text-brand-ink-hint break-words">{s.label}</p>
                          <p className={`text-base font-black leading-tight ${s.accent}`}>{s.val}</p>
                          <p className="mt-0.5 text-[9px] text-brand-ink-hint hidden sm:block break-words">{s.sub}</p>
                        </motion.div>
                      ))}
                    </div>

                    <div className="mt-3 grid min-w-0 gap-3 sm:grid-cols-[1fr_0.85fr]">
                      <div>
                        <div className="mb-2 flex min-w-0 items-center justify-between">
                          <span className="text-[10px] font-black uppercase tracking-wider text-brand-ink-muted break-words">Room Board</span>
                          <div className="flex min-w-0 gap-2">
                            {Object.entries(ROOM_STATUS_MAP).map(([key, val]) => (
                              <div key={key} className="flex min-w-0 items-center gap-1">
                                ...
                                <span className="text-[8px] text-brand-ink-hint break-words">{val.label}</span>
                              </div>
                            ))}
                          </div>
                        </div>
                        <motion.div
                          ...
                          className="grid min-w-0 grid-cols-8 gap-1"
                        >
                          {ROOM_GRID.map((r) => (
                            <motion.div
                              key={r.num}
                      ...
                      <div>
                        <span className="mb-2 block text-[10px] font-black uppercase tracking-wider text-brand-ink-muted break-words">Live Activity</span>
                        <motion.div
                          initial="hidden"
                          animate="visible"
                          variants={{
                        ...
                        >
                          {TODAY_ACTIVITY.map((a, i) => (
                            <motion.div
                              ...
                              className="group flex min-w-0 items-start gap-2 rounded-xl border border-brand-surface-border bg-brand-surface-light/40 px-2 py-1.5 transition-colors hover:bg-white"
                            >
                              <motion.span
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <p className="text-[9px] text-brand-ink-hint break-words">{a.time} AM</p>
                              </div>
                            </motion.div>
                          ))}
                        </motion.div>
      ...
      <section id="features" className="scroll-mt-20 border-t border-brand-surface-border bg-white">
        <div className="mx-auto max-w-7xl px-5 py-12 sm:px-8 lg:px-10 lg:py-16">
          ...
          <motion.div
            ...
            className="mt-8 grid min-w-0 gap-4 sm:grid-cols-2 lg:grid-cols-4"
          >
            {MODULES.map((m) => {
              const Icon = m.icon;
              return (
                ...
                >
                  ...
                  <h3 className="mt-3 text-sm font-black text-brand-ink break-words">{t(`m.home.modules.${m.id}.title`)}</h3>
                  <p className="mt-1 text-[13px] font-medium leading-[1.5] text-brand-ink-muted break-words">{t(`m.home.modules.${m.id}.desc`)}</p>
        ...
        <div className="mx-auto max-w-7xl px-5 py-12 sm:px-8 lg:px-10 lg:py-16">
          ...
          <div className="relative mt-10">
            ...
            <motion.div
              ...
              className="grid min-w-0 grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6"
            >
              {WORKFLOW.map((step, i) => {
                ...
                return (
                  ...
                  >
                    <motion.div
                      ...
                      className="relative z-10 flex min-w-0 h-[60px] w-[60px] items-center justify-center rounded-2xl border-2 border-brand-surface-border bg-white text-brand-ink shadow-sm transition-all group-hover:border-brand-primary/40 group-hover:shadow-md"
                    >
                      <Icon className="h-6 w-6 shrink-0" />
                      <span className="absolute -right-1 -top-1 flex min-w-0 h-5 w-5 items-center justify-center rounded-full bg-brand-primary text-[10px] font-bold text-white shadow-sm break-words">
                        {i + 1}
                      </span>
                    </motion.div>
                    <p className="mt-2.5 text-sm font-black text-brand-ink break-words">{t(`m.home.wf.${step.id}.label`)}</p>
                    <p className="text-[11px] font-medium text-brand-ink-hint break-words">{t(`m.home.wf.${step.id}.sub`)}</p>
                  </motion.div>
                );
              })}
            </motion.div>
        ...
        <div className="mx-auto max-w-7xl px-5 py-12 sm:px-8 lg:px-10 lg:py-16">
          <div className="lg:flex lg:items-center lg:gap-14">
            <Reveal className="lg:w-[340px] lg:flex-shrink-0">
              <p className="text-[11px] font-black uppercase tracking-[0.24em] text-brand-ink-hint break-words">{t('m.home.capsEyebrow')}</p>
              <h2 className="mt-1.5 text-xl font-black tracking-tight text-brand-ink sm:text-2xl break-words">
                {t('m.home.capsTitle')}
              </h2>
              <p className="mt-2 text-[0.85rem] font-medium leading-relaxed text-brand-ink-muted break-words">
                {t('m.home.capsBio')}
              </p>
              <Link
                to="/login"
                className="group mt-5 inline-flex min-w-0 items-center gap-1.5 text-sm font-bold text-brand-ink transition hover:text-brand-ink"
              >
                {t('m.home.capsCta')}
                <ArrowRight className="h-3.5 w-3.5 transition-transform group-hover:translate-x-1 rtl:rotate-180 shrink-0" />
              </Link>
            </Reveal>
            ...
            <motion.div
              ...
              className="mt-8 grid min-w-0 flex-1 gap-3 sm:grid-cols-2 lg:mt-0"
            >
              {CAPABILITIES.map((cap) => {
                ...
                return (
                  <motion.div
                    ...
                    transition={{ duration: 0.55, ease: EASE }}
                    whileHover={hoverLift}
                    className="flex min-w-0 gap-3 rounded-[1.35rem] border border-brand-surface-border bg-brand-surface-light p-4 transition-all hover:border-brand-primary/30 hover:bg-white hover:shadow-sm"
                  >
                    <div className="flex min-w-0 h-9 w-9 flex-shrink-0 items-center justify-center rounded-2xl bg-brand-primary-tint text-brand-ink">
                      <Icon className="h-[18px] w-[18px] shrink-0" />
                    </div>
                    <div>
                      <p className="text-sm font-black text-brand-ink break-words">{t(`m.home.cap.${cap.id}.label`)}</p>
                      <p className="mt-0.5 text-[12px] font-medium leading-snug text-brand-ink-muted break-words">{t(`m.home.cap.${cap.id}.desc`)}</p>
                    </div>
                  </motion.div>
                );
              })}
        ...
        <div className="mx-auto max-w-7xl px-5 py-10 sm:px-8 lg:px-10">
          ...
          >
            <div>
              ...
              <p className="mt-1 text-sm font-medium text-brand-ink-hint break-words">{t('m.home.banner.sub')}</p>
            </div>
            <div className="flex min-w-0 gap-3">
              ...
              <motion.div whileHover={reduceMotion ? {} : { scale: 1.04 }} whileTap={{ scale: 0.96 }}>
                <Link
                  to="/bookings"
                  className="group inline-flex min-w-0 items-center gap-1.5 rounded-full border border-white/20 px-5 py-2.5 text-sm font-bold text-white transition hover:bg-white/10"
                >
