  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6 lg:p-8">
      <div className="rounded-[1.5rem] border border-brand-primary/20 bg-white p-5">
        ...
        <h1 className="mt-2 text-3xl font-black tracking-tight text-brand-ink">Payment history</h1>
      </div>
              ...
              {visiblePayments.map((payment) => (
                <tr key={payment.paymentId} className="border-b border-brand-surface-border">
                  ...
                  <td className="px-3 py-3">
                    <span className="rounded-full bg-brand-surface-light px-3 py-1 text-xs font-black">{payment.paymentStatus}</span>
                  </td>
                  ...
                  <td className="px-3 py-3">
                    <div className="flex flex-wrap gap-2">
                      <button
                        ...
                        className="inline-flex items-center gap-1 rounded-full border border-brand-surface-border px-3 py-2 text-xs font-bold"
                      >
                      ...
                      {isRefundable(payment) ? (
                        <button
                          ...
                          className="inline-flex items-center gap-1 rounded-full border border-brand-danger/40 px-3 py-2 text-xs font-bold text-brand-danger disabled:opacity-60"
                        >
      ...
      {selectedReceipt ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-auto rounded-[1.5rem] bg-white p-5">
            <div className="flex items-start justify-between gap-4 border-b border-brand-surface-border pb-4">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.2em] text-brand-primary">Roomify Payment Receipt</p>
                <h2 className="mt-2 text-xl font-black text-brand-ink">{selectedReceipt.gatewayReference}</h2>
              </div>
              <button type="button" onClick={() => setSelectedReceipt(null)} className="rounded-full border px-3 py-2 text-sm font-bold">
                Close
              </button>
            </div>
            ...
            <div className="mt-4 flex gap-3">
              <button type="button" onClick={() => window.print()} className="rounded-full bg-brand-primary px-5 py-3 text-sm font-bold text-white">
                Print Receipt
              </button>
              <button type="button" onClick={() => setSelectedReceipt(null)} className="inline-flex items-center gap-2 rounded-full border px-5 py-3 text-sm font-bold">
