...
public class BillingService {
        ...
        public BillResponse recordPayment(String confirmationNumber, BigDecimal amount) {
                ...
                boolean isFullyPaid = projectedOutstanding
                                .compareTo(BigDecimal.ZERO.setScale(MONEY_SCALE, ROUNDING)) == 0;
        ...
        private BillResponse buildBillResponse(
                ...
                if (auditBillCalculated) {
                        ...
                        log.info(
                                        ...
                                        normalizedConfirmation, nights, roomCharge, safeServiceCharges, vatAmount,
                                        safeDiscount, balanceDue);
                }
        ...
        private List<BillLineItem> buildLineItems(
                ...
                items.add(new BillLineItem(
                                String.format("Room Charge (%d night%s x %s)", nights, nights == 1 ? "" : "s",
                                                roomRate),
                                roomCharge,
                                false));
        ...
        private BigDecimal calculateOutstanding(BigDecimal totalPrice, BigDecimal totalPaid) {
                BigDecimal outstanding = sanitise(totalPrice).subtract(sanitise(totalPaid)).setScale(MONEY_SCALE,
                                ROUNDING);
